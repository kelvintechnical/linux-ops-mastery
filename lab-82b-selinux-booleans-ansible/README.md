# Lab 82b - Toggling SELinux Booleans (Ansible)
...
## Task 1 - Ansible equivalent for Toggling SELinux Booleans
...
printf 'lab 82b task 1 warm-up at %s
...
Ansible equivalent for Toggling SELinux Booleans. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-82b.yml" <<'EOF'
---
- name: Lab 82b Task 1 - Toggling SELinux Booleans
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Query SELinux state
      ...
      changed_when: false
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-82b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 82b task 1: PASS
...
Lab 82b Task 1
What I practiced: Ansible equivalent for Toggling SELinux Booleans
...
## Task 2 - Ansible trap rehearsal for Toggling SELinux Booleans
...
ls -ld /sbin
test -e /sbin && echo "practice directory exists"
printf 'lab 82b task 2 warm-up at %s
...
Ansible trap rehearsal for Toggling SELinux Booleans. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-82b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 82b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 82b Task 2
What I practiced: Ansible trap rehearsal for Toggling SELinux Booleans
