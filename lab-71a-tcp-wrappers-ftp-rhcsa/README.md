# Lab 71a - Configure TCP Wrappers for FTP (RHCSA)
...
- PRIMARY COMMANDS: `vsftpd, /etc/hosts.deny`
- TRAPS: T71-A: vsftpd may not be libwrap-enabled on modern builds; T71-B: starting a service and opening firewall are separate actions; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=71
export LAB_SLUG=tcp_wrappers_ftp
...
Practice directory: /tmp
Temporary files live here and are cleared by policy. It is the safe place for repeatable lab sandboxes.
EOF
id "${USER}"
ls -ld "${SANDBOX}" "${USER_HOME}" /tmp
echo "Sandbox built by $(whoami) at $(date -Is)"
echo "exit was: $?"
```

## Task 1 - Canonical RHCSA form for Configure TCP Wrappers for FTP
...
pwd
ls -ld /tmp
test -e /tmp && echo "practice directory exists"
printf 'lab 71a task 1 warm-up at %s
...
Canonical RHCSA form for Configure TCP Wrappers for FTP. This task connects `vsftpd, /etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 71 evidence for Configure TCP Wrappers for FTP\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: vsftpd, /etc/hosts.deny\n' | tee -a ${SANDBOX}/task-1-topic.txt
ls -ld /tmp | tee -a "${SANDBOX}/task-1-practice.txt"
test -e /tmp && echo "/tmp checked again" | tee -a "${SANDBOX}/task-1-practice.txt"
...
Task 1 is the safe, exam-style version of Configure TCP Wrappers for FTP. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `vsftpd` | Practice token for this lab | Repeat it until recognition is automatic |
| `/etc/hosts.deny` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `vsftpd, /etc/hosts.deny` |
| Practice directory | `/tmp` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T71-A: vsftpd may not be libwrap-enabled on modern builds |
...
printf 'Persistence check 71a task 1: PASS
...
Lab 71a Task 1
What I practiced: Canonical RHCSA form for Configure TCP Wrappers for FTP
Trap rehearsed: T71-A: vsftpd may not be libwrap-enabled on modern builds
...
## Task 2 - Trap rehearsal and contrast for Configure TCP Wrappers for FTP
...
ls -ld /tmp
test -e /tmp && echo "practice directory exists"
printf 'lab 71a task 2 warm-up at %s
...
Trap rehearsal and contrast for Configure TCP Wrappers for FTP. This task connects `vsftpd, /etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T71-B: starting a service and opening firewall are separate actions\n' | tee ${SANDBOX}/task-2-trap.txt
printf 'Restore note: verify before cleanup\n' | tee -a ${SANDBOX}/task-2-trap.txt
ls -ld /tmp | tee -a "${SANDBOX}/task-2-practice.txt"
test -e /tmp && echo "/tmp checked again" | tee -a "${SANDBOX}/task-2-practice.txt"
...
| Practice directory | `/tmp` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T71-B: starting a service and opening firewall are separate actions |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 71a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 71a Task 2
What I practiced: Trap rehearsal and contrast for Configure TCP Wrappers for FTP
Trap rehearsed: T71-B: starting a service and opening firewall are separate actions
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 71 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
