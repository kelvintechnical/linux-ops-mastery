# Lab 87b - Generate New GRUB Config (Ansible)
...
## Task 1 - Ansible equivalent for Generate New GRUB Config
...
printf 'lab 87b task 1 warm-up at %s
...
Ansible equivalent for Generate New GRUB Config. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-87b.yml" <<'EOF'
---
- name: Lab 87b Task 1 - Generate New GRUB Config
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Preview GRUB setting safely
      ansible.builtin.lineinfile:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-87b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 87b task 1: PASS
...
Lab 87b Task 1
What I practiced: Ansible equivalent for Generate New GRUB Config
...
## Task 2 - Ansible trap rehearsal for Generate New GRUB Config
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 87b task 2 warm-up at %s
...
Ansible trap rehearsal for Generate New GRUB Config. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-87b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 87b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 87b Task 2
What I practiced: Ansible trap rehearsal for Generate New GRUB Config
