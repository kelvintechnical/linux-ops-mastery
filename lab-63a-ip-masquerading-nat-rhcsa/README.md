# Lab 63a - Configure IP Masquerading NAT (RHCSA)
...
- PRIMARY COMMANDS: `firewall-cmd --add-masquerade`
- TRAPS: T63-A: masquerade on the wrong zone produces no NAT for the expected interface; T63-B: masquerade is useless without IP forwarding when routing between networks; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=63
export LAB_SLUG=ip_masquerading_nat
...
## Task 1 - Canonical RHCSA form for Configure IP Masquerading NAT
...
printf 'lab 63a task 1 warm-up at %s
...
Canonical RHCSA form for Configure IP Masquerading NAT. This task connects `firewall-cmd --add-masquerade` to a repeatable artifact in `${SANDBOX}`.
...
sudo firewall-cmd --zone=external --query-masquerade | tee ${SANDBOX}/masq.txt
...
Task 1 is the safe, exam-style version of Configure IP Masquerading NAT. It creates evidence first and avoids changing production state without a restore path.
...
| `--add-masquerade` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `firewall-cmd --add-masquerade` |
| Practice directory | `/lib` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T63-A: masquerade on the wrong zone produces no NAT for the expected interface |
...
printf 'Persistence check 63a task 1: PASS
...
Lab 63a Task 1
What I practiced: Canonical RHCSA form for Configure IP Masquerading NAT
Trap rehearsed: T63-A: masquerade on the wrong zone produces no NAT for the expected interface
...
## Task 2 - Trap rehearsal and contrast for Configure IP Masquerading NAT
...
ls -ld /lib
test -e /lib && echo "practice directory exists"
printf 'lab 63a task 2 warm-up at %s
...
Trap rehearsal and contrast for Configure IP Masquerading NAT. This task connects `firewall-cmd --add-masquerade` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sysctl net.ipv4.ip_forward | tee ${SANDBOX}/forwarding-check.txt
...
| Practice directory | `/lib` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T63-B: masquerade is useless without IP forwarding when routing between networks |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 63a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 63a Task 2
What I practiced: Trap rehearsal and contrast for Configure IP Masquerading NAT
Trap rehearsed: T63-B: masquerade is useless without IP forwarding when routing between networks
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 63 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
