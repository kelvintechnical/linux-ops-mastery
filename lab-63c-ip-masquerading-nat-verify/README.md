# Lab 63c - Configure IP Masquerading NAT (Verify)
...
## Task 1 - Audit evidence for Configure IP Masquerading NAT
...
printf 'lab 63c task 1 warm-up at %s
...
Audit evidence for Configure IP Masquerading NAT. This task connects `firewall-cmd --add-masquerade` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 63c task 1: PASS
...
Lab 63c Task 1
What I practiced: Audit evidence for Configure IP Masquerading NAT
...
## Task 2 - Destroy-restore drill for Configure IP Masquerading NAT
...
ls -ld /lib
test -e /lib && echo "practice directory exists"
printf 'lab 63c task 2 warm-up at %s
...
Destroy-restore drill for Configure IP Masquerading NAT. This task connects `firewall-cmd --add-masquerade` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-63-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-63-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 63c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 63c Task 2
What I practiced: Destroy-restore drill for Configure IP Masquerading NAT
