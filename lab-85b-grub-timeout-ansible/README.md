# Lab 85b - Modify GRUB Timeout (Ansible)
...
## Task 1 - Ansible equivalent for Modify GRUB Timeout
...
printf 'lab 85b task 1 warm-up at %s
...
Ansible equivalent for Modify GRUB Timeout. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-85b.yml" <<'EOF'
---
- name: Lab 85b Task 1 - Modify GRUB Timeout
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Preview GRUB setting safely
      ansible.builtin.lineinfile:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-85b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 85b task 1: PASS
...
Lab 85b Task 1
What I practiced: Ansible equivalent for Modify GRUB Timeout
...
## Task 2 - Ansible trap rehearsal for Modify GRUB Timeout
...
ls -ld /usr
test -e /usr && echo "practice directory exists"
printf 'lab 85b task 2 warm-up at %s
...
Ansible trap rehearsal for Modify GRUB Timeout. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-85b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 85b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 85b Task 2
What I practiced: Ansible trap rehearsal for Modify GRUB Timeout
