# Lab 78a - Managing SELinux Modes (RHCSA)
...
## Task 1 - Canonical RHCSA form for Managing SELinux Modes
...
printf 'lab 78a task 1 warm-up at %s
...
Canonical RHCSA form for Managing SELinux Modes. This task connects `sestatus, setenforce` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sestatus | tee ${SANDBOX}/sestatus.txt
getenforce | tee ${SANDBOX}/getenforce.txt
...
Task 1 is the safe, exam-style version of Managing SELinux Modes. It creates evidence first and avoids changing production state without a restore path.
...
printf 'Persistence check 78a task 1: PASS
...
Lab 78a Task 1
What I practiced: Canonical RHCSA form for Managing SELinux Modes
...
## Task 2 - Trap rehearsal and contrast for Managing SELinux Modes
...
ls -ld /media
test -e /media && echo "practice directory exists"
printf 'lab 78a task 2 warm-up at %s
...
Trap rehearsal and contrast for Managing SELinux Modes. This task connects `sestatus, setenforce` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sudo setenforce 0 || true
getenforce | tee ${SANDBOX}/after-setenforce.txt
...
printf 'Persistence check 78a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 78a Task 2
What I practiced: Trap rehearsal and contrast for Managing SELinux Modes
