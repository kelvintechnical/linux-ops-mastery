# Lab 79a - Viewing SELinux Contexts (RHCSA)
...
## Task 1 - Canonical RHCSA form for Viewing SELinux Contexts
...
printf 'lab 79a task 1 warm-up at %s
...
Canonical RHCSA form for Viewing SELinux Contexts. This task connects `ls -Z, ps -eZ` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 79 evidence for Viewing SELinux Contexts\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: ls -Z, ps -eZ\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Viewing SELinux Contexts. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `ls` | Practice token for this lab | Repeat it until recognition is automatic |
| `-Z` | Practice token for this lab | Repeat it until recognition is automatic |
| `ps` | Practice token for this lab | Repeat it until recognition is automatic |
| `-eZ` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `ls -Z, ps -eZ` |
| Practice directory | `/mnt` |
...
printf 'Persistence check 79a task 1: PASS
...
Lab 79a Task 1
What I practiced: Canonical RHCSA form for Viewing SELinux Contexts
...
## Task 2 - Trap rehearsal and contrast for Viewing SELinux Contexts
...
ls -ld /mnt
test -e /mnt && echo "practice directory exists"
printf 'lab 79a task 2 warm-up at %s
...
Trap rehearsal and contrast for Viewing SELinux Contexts. This task connects `ls -Z, ps -eZ` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T79-B: process contexts and file contexts are different object classes\n' | tee ${SANDBOX}/task-2-trap.txt
...
printf 'Persistence check 79a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 79a Task 2
What I practiced: Trap rehearsal and contrast for Viewing SELinux Contexts
