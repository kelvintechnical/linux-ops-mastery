# Lab 88b - Reset Root Password via Boot (Ansible)
...
## Task 1 - Ansible equivalent for Reset Root Password via Boot
...
printf 'lab 88b task 1 warm-up at %s
...
Ansible equivalent for Reset Root Password via Boot. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-88b.yml" <<'EOF'
---
- name: Lab 88b Task 1 - Reset Root Password via Boot
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Preview GRUB setting safely
      ansible.builtin.lineinfile:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-88b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 88b task 1: PASS
...
Lab 88b Task 1
What I practiced: Ansible equivalent for Reset Root Password via Boot
...
## Task 2 - Ansible trap rehearsal for Reset Root Password via Boot
...
ls -ld /home
test -e /home && echo "practice directory exists"
printf 'lab 88b task 2 warm-up at %s
...
Ansible trap rehearsal for Reset Root Password via Boot. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-88b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 88b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 88b Task 2
What I practiced: Ansible trap rehearsal for Reset Root Password via Boot
