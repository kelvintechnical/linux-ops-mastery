# Lab 81b - Persistent Context Restoration (Ansible)
...
## Task 1 - Ansible equivalent for Persistent Context Restoration
...
printf 'lab 81b task 1 warm-up at %s
...
Ansible equivalent for Persistent Context Restoration. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-81b.yml" <<'EOF'
---
- name: Lab 81b Task 1 - Persistent Context Restoration
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Query SELinux state
      ...
      changed_when: false
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-81b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 81b task 1: PASS
...
Lab 81b Task 1
What I practiced: Ansible equivalent for Persistent Context Restoration
...
## Task 2 - Ansible trap rehearsal for Persistent Context Restoration
...
ls -ld /bin
test -e /bin && echo "practice directory exists"
printf 'lab 81b task 2 warm-up at %s
...
Ansible trap rehearsal for Persistent Context Restoration. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-81b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 81b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 81b Task 2
What I practiced: Ansible trap rehearsal for Persistent Context Restoration
