# Lab 76b - Use PAM to Limit User Access (Ansible)
...
## Task 1 - Ansible equivalent for Use PAM to Limit User Access
...
printf 'lab 76b task 1 warm-up at %s
...
Ansible equivalent for Use PAM to Limit User Access. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-76b.yml" <<'EOF'
---
- name: Lab 76b Task 1 - Use PAM to Limit User Access
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Stage PAM example safely
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-76b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 76b task 1: PASS
...
Lab 76b Task 1
What I practiced: Ansible equivalent for Use PAM to Limit User Access
...
## Task 2 - Ansible trap rehearsal for Use PAM to Limit User Access
...
ls -ld /sys
test -e /sys && echo "practice directory exists"
printf 'lab 76b task 2 warm-up at %s
...
Ansible trap rehearsal for Use PAM to Limit User Access. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-76b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 76b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 76b Task 2
What I practiced: Ansible trap rehearsal for Use PAM to Limit User Access
