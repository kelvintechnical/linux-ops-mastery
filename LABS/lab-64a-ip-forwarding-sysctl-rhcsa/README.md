# Lab 64a - Configure IP Forwarding (RHCSA)
...
- PRIMARY COMMANDS: `/etc/sysctl.conf, sysctl -p`
- TRAPS: T64-A: echoing into /proc/sys is runtime-only and disappears after reboot; T64-B: duplicate sysctl keys hide the value you think is active; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=64
export LAB_SLUG=ip_forwarding_sysctl
...
## Task 1 - Canonical RHCSA form for Configure IP Forwarding
...
printf 'lab 64a task 1 warm-up at %s
...
Canonical RHCSA form for Configure IP Forwarding. This task connects `/etc/sysctl.conf, sysctl -p` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sysctl net.ipv4.ip_forward | tee ${SANDBOX}/before.txt
echo 'net.ipv4.ip_forward = 1' | sudo tee ${SANDBOX}/99-lab64-ip-forward.conf
sudo sysctl -p ${SANDBOX}/99-lab64-ip-forward.conf | tee ${SANDBOX}/after.txt
...
Task 1 is the safe, exam-style version of Configure IP Forwarding. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `/etc/sysctl.conf` | Practice token for this lab | Repeat it until recognition is automatic |
| `sysctl` | Practice token for this lab | Repeat it until recognition is automatic |
| `-p` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `/etc/sysctl.conf, sysctl -p` |
| Practice directory | `/lib64` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T64-A: echoing into /proc/sys is runtime-only and disappears after reboot |
...
printf 'Persistence check 64a task 1: PASS
...
Lab 64a Task 1
What I practiced: Canonical RHCSA form for Configure IP Forwarding
Trap rehearsed: T64-A: echoing into /proc/sys is runtime-only and disappears after reboot
...
## Task 2 - Trap rehearsal and contrast for Configure IP Forwarding
...
ls -ld /lib64
test -e /lib64 && echo "practice directory exists"
printf 'lab 64a task 2 warm-up at %s
...
Trap rehearsal and contrast for Configure IP Forwarding. This task connects `/etc/sysctl.conf, sysctl -p` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sudo sysctl -w net.ipv4.ip_forward=1 | tee ${SANDBOX}/runtime-only.txt
sysctl net.ipv4.ip_forward | tee -a ${SANDBOX}/runtime-only.txt
...
| Practice directory | `/lib64` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T64-B: duplicate sysctl keys hide the value you think is active |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 64a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 64a Task 2
What I practiced: Trap rehearsal and contrast for Configure IP Forwarding
Trap rehearsed: T64-B: duplicate sysctl keys hide the value you think is active
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 64 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
