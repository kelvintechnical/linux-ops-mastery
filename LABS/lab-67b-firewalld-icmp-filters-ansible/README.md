# Lab 67b - Configure ICMP Filters (Ansible)
...
## Task 1 - Ansible equivalent for Configure ICMP Filters
...
printf 'lab 67b task 1 warm-up at %s
...
Ansible equivalent for Configure ICMP Filters. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-67b.yml" <<'EOF'
---
- name: Lab 67b Task 1 - Configure ICMP Filters
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Manage firewalld state safely
      ansible.posix.firewalld:
        ...
        state: enabled
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-67b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 67b task 1: PASS
...
Lab 67b Task 1
What I practiced: Ansible equivalent for Configure ICMP Filters
...
## Task 2 - Ansible trap rehearsal for Configure ICMP Filters
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 67b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure ICMP Filters. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-67b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 67b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 67b Task 2
What I practiced: Ansible trap rehearsal for Configure ICMP Filters
