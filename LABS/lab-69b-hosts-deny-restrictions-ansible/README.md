# Lab 69b - Restrict Access via hosts.deny (Ansible)
...
## Task 1 - Ansible equivalent for Restrict Access via hosts.deny
...
printf 'lab 69b task 1 warm-up at %s
...
Ansible equivalent for Restrict Access via hosts.deny. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-69b.yml" <<'EOF'
---
- name: Lab 69b Task 1 - Restrict Access via hosts.deny
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Write wrapper file from a template copy
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-69b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 69b task 1: PASS
...
Lab 69b Task 1
What I practiced: Ansible equivalent for Restrict Access via hosts.deny
...
## Task 2 - Ansible trap rehearsal for Restrict Access via hosts.deny
...
ls -ld /root
test -e /root && echo "practice directory exists"
printf 'lab 69b task 2 warm-up at %s
...
Ansible trap rehearsal for Restrict Access via hosts.deny. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-69b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 69b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 69b Task 2
What I practiced: Ansible trap rehearsal for Restrict Access via hosts.deny
