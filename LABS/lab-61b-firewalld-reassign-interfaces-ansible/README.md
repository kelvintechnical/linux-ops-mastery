# Lab 61b - Reassign Interfaces to Zones (Ansible)
...
## Task 1 - Ansible equivalent for Reassign Interfaces to Zones
...
printf 'lab 61b task 1 warm-up at %s
...
Ansible equivalent for Reassign Interfaces to Zones. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-61b.yml" <<'EOF'
---
- name: Lab 61b Task 1 - Reassign Interfaces to Zones
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Manage firewalld state safely
      ansible.posix.firewalld:
        ...
        state: enabled
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-61b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 61b task 1: PASS
...
Lab 61b Task 1
What I practiced: Ansible equivalent for Reassign Interfaces to Zones
...
## Task 2 - Ansible trap rehearsal for Reassign Interfaces to Zones
...
ls -ld /bin
test -e /bin && echo "practice directory exists"
printf 'lab 61b task 2 warm-up at %s
...
Ansible trap rehearsal for Reassign Interfaces to Zones. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-61b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 61b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 61b Task 2
What I practiced: Ansible trap rehearsal for Reassign Interfaces to Zones
