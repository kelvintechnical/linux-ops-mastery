# Lab 64c - Configure IP Forwarding (Verify)
...
## Task 1 - Audit evidence for Configure IP Forwarding
...
printf 'lab 64c task 1 warm-up at %s
...
Audit evidence for Configure IP Forwarding. This task connects `/etc/sysctl.conf, sysctl -p` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 64c task 1: PASS
...
Lab 64c Task 1
What I practiced: Audit evidence for Configure IP Forwarding
...
## Task 2 - Destroy-restore drill for Configure IP Forwarding
...
ls -ld /lib64
test -e /lib64 && echo "practice directory exists"
printf 'lab 64c task 2 warm-up at %s
...
Destroy-restore drill for Configure IP Forwarding. This task connects `/etc/sysctl.conf, sysctl -p` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-64-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-64-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 64c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 64c Task 2
What I practiced: Destroy-restore drill for Configure IP Forwarding
