# Lab 70a - Allow Specific Access via hosts.allow (RHCSA)
...
## Task 1 - Canonical RHCSA form for Allow Specific Access via hosts.allow
...
printf 'lab 70a task 1 warm-up at %s
...
Canonical RHCSA form for Allow Specific Access via hosts.allow. This task connects `/etc/hosts.allow` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 70 evidence for Allow Specific Access via hosts.allow\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: /etc/hosts.allow\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Allow Specific Access via hosts.allow. It creates evidence first and avoids changing production state without a restore path.
...
printf 'Persistence check 70a task 1: PASS
...
Lab 70a Task 1
What I practiced: Canonical RHCSA form for Allow Specific Access via hosts.allow
...
## Task 2 - Trap rehearsal and contrast for Allow Specific Access via hosts.allow
...
ls -ld /var
test -e /var && echo "practice directory exists"
printf 'lab 70a task 2 warm-up at %s
...
Trap rehearsal and contrast for Allow Specific Access via hosts.allow. This task connects `/etc/hosts.allow` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T70-B: broad ALL : ALL in hosts.allow defeats the deny file\n' | tee ${SANDBOX}/task-2-trap.txt
...
printf 'Persistence check 70a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 70a Task 2
What I practiced: Trap rehearsal and contrast for Allow Specific Access via hosts.allow
