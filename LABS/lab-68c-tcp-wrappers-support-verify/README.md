# Lab 68c - Verify TCP Wrappers Support (Verify)
...
## Task 1 - Audit evidence for Verify TCP Wrappers Support
...
printf 'lab 68c task 1 warm-up at %s
...
Audit evidence for Verify TCP Wrappers Support. This task connects `ldd /usr/sbin/sshd | grep libwrap` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 68c task 1: PASS
...
Lab 68c Task 1
What I practiced: Audit evidence for Verify TCP Wrappers Support
...
## Task 2 - Destroy-restore drill for Verify TCP Wrappers Support
...
ls -ld /home
test -e /home && echo "practice directory exists"
printf 'lab 68c task 2 warm-up at %s
...
Destroy-restore drill for Verify TCP Wrappers Support. This task connects `ldd /usr/sbin/sshd | grep libwrap` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-68-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-68-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 68c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 68c Task 2
What I practiced: Destroy-restore drill for Verify TCP Wrappers Support
