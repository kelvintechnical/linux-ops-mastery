# Lab 88a - Reset Root Password via Boot (RHCSA)
...
- PRIMARY COMMANDS: `rd.break, chroot, passwd`
- TRAPS: T88-A: forgetting touch /.autorelabel can leave SELinux labels wrong; T88-B: mounting sysroot read-only prevents passwd from saving; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=88
export LAB_SLUG=grub_reset_root_password
...
## Task 1 - Canonical RHCSA form for Reset Root Password via Boot
...
printf 'lab 88a task 1 warm-up at %s
...
Canonical RHCSA form for Reset Root Password via Boot. This task connects `rd.break, chroot, passwd` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 88 evidence for Reset Root Password via Boot\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: rd.break, chroot, passwd\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Reset Root Password via Boot. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `rd.break` | Practice token for this lab | Repeat it until recognition is automatic |
| `chroot` | Practice token for this lab | Repeat it until recognition is automatic |
| `passwd` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `rd.break, chroot, passwd` |
| Practice directory | `/home` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T88-A: forgetting touch /.autorelabel can leave SELinux labels wrong |
...
printf 'Persistence check 88a task 1: PASS
...
Lab 88a Task 1
What I practiced: Canonical RHCSA form for Reset Root Password via Boot
Trap rehearsed: T88-A: forgetting touch /.autorelabel can leave SELinux labels wrong
...
## Task 2 - Trap rehearsal and contrast for Reset Root Password via Boot
...
ls -ld /home
test -e /home && echo "practice directory exists"
printf 'lab 88a task 2 warm-up at %s
...
Trap rehearsal and contrast for Reset Root Password via Boot. This task connects `rd.break, chroot, passwd` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T88-B: mounting sysroot read-only prevents passwd from saving\n' | tee ${SANDBOX}/task-2-trap.txt
...
| Practice directory | `/home` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T88-B: mounting sysroot read-only prevents passwd from saving |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 88a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 88a Task 2
What I practiced: Trap rehearsal and contrast for Reset Root Password via Boot
Trap rehearsed: T88-B: mounting sysroot read-only prevents passwd from saving
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 88 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
