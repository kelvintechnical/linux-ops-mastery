# Lab 62c - Allow Services Through Firewall (Verify)
...
## Task 1 - Audit evidence for Allow Services Through Firewall
...
printf 'lab 62c task 1 warm-up at %s
...
Audit evidence for Allow Services Through Firewall. This task connects `firewall-cmd --permanent --add-service` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 62c task 1: PASS
...
Lab 62c Task 1
What I practiced: Audit evidence for Allow Services Through Firewall
...
## Task 2 - Destroy-restore drill for Allow Services Through Firewall
...
ls -ld /sbin
test -e /sbin && echo "practice directory exists"
printf 'lab 62c task 2 warm-up at %s
...
Destroy-restore drill for Allow Services Through Firewall. This task connects `firewall-cmd --permanent --add-service` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-62-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-62-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 62c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 62c Task 2
What I practiced: Destroy-restore drill for Allow Services Through Firewall
