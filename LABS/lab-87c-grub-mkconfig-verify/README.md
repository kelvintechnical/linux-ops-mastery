# Lab 87c - Generate New GRUB Config (Verify)
...
## Task 1 - Audit evidence for Generate New GRUB Config
...
printf 'lab 87c task 1 warm-up at %s
...
Audit evidence for Generate New GRUB Config. This task connects `grub2-mkconfig -o /boot/grub2/grub.cfg` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 87c task 1: PASS
...
Lab 87c Task 1
What I practiced: Audit evidence for Generate New GRUB Config
...
## Task 2 - Destroy-restore drill for Generate New GRUB Config
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 87c task 2 warm-up at %s
...
Destroy-restore drill for Generate New GRUB Config. This task connects `grub2-mkconfig -o /boot/grub2/grub.cfg` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-87-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-87-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 87c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 87c Task 2
What I practiced: Destroy-restore drill for Generate New GRUB Config
