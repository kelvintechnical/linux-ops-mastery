# Lab 79c - Viewing SELinux Contexts (Verify)
...
## Task 1 - Audit evidence for Viewing SELinux Contexts
...
printf 'lab 79c task 1 warm-up at %s
...
Audit evidence for Viewing SELinux Contexts. This task connects `ls -Z, ps -eZ` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 79c task 1: PASS
...
Lab 79c Task 1
What I practiced: Audit evidence for Viewing SELinux Contexts
...
## Task 2 - Destroy-restore drill for Viewing SELinux Contexts
...
ls -ld /mnt
test -e /mnt && echo "practice directory exists"
printf 'lab 79c task 2 warm-up at %s
...
Destroy-restore drill for Viewing SELinux Contexts. This task connects `ls -Z, ps -eZ` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-79-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-79-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 79c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 79c Task 2
What I practiced: Destroy-restore drill for Viewing SELinux Contexts
