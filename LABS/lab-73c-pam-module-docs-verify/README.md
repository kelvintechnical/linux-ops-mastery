# Lab 73c - Read PAM Module Documentation (Verify)
...
## Task 1 - Audit evidence for Read PAM Module Documentation
...
printf 'lab 73c task 1 warm-up at %s
...
Audit evidence for Read PAM Module Documentation. This task connects `/usr/share/doc/pam-*/txts/` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 73c task 1: PASS
...
Lab 73c Task 1
What I practiced: Audit evidence for Read PAM Module Documentation
...
## Task 2 - Destroy-restore drill for Read PAM Module Documentation
...
ls -ld /srv
test -e /srv && echo "practice directory exists"
printf 'lab 73c task 2 warm-up at %s
...
Destroy-restore drill for Read PAM Module Documentation. This task connects `/usr/share/doc/pam-*/txts/` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-73-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-73-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 73c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 73c Task 2
What I practiced: Destroy-restore drill for Read PAM Module Documentation
