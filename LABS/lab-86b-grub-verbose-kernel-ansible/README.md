# Lab 86b - Enable Verbose Kernel Messages (Ansible)
...
## Task 1 - Ansible equivalent for Enable Verbose Kernel Messages
...
printf 'lab 86b task 1 warm-up at %s
...
Ansible equivalent for Enable Verbose Kernel Messages. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-86b.yml" <<'EOF'
---
- name: Lab 86b Task 1 - Enable Verbose Kernel Messages
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Preview GRUB setting safely
      ansible.builtin.lineinfile:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-86b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 86b task 1: PASS
...
Lab 86b Task 1
What I practiced: Ansible equivalent for Enable Verbose Kernel Messages
...
## Task 2 - Ansible trap rehearsal for Enable Verbose Kernel Messages
...
ls -ld /etc
test -e /etc && echo "practice directory exists"
printf 'lab 86b task 2 warm-up at %s
...
Ansible trap rehearsal for Enable Verbose Kernel Messages. This task connects `ansible.builtin.lineinfile plus ansible.builtin.command grub2-mkconfig boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-86b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 86b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 86b Task 2
What I practiced: Ansible trap rehearsal for Enable Verbose Kernel Messages
