# Lab 77b - Restrict Service Access by User List (Ansible)
...
## Task 1 - Ansible equivalent for Restrict Service Access by User List
...
printf 'lab 77b task 1 warm-up at %s
...
Ansible equivalent for Restrict Service Access by User List. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-77b.yml" <<'EOF'
---
- name: Lab 77b Task 1 - Restrict Service Access by User List
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Stage PAM example safely
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-77b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 77b task 1: PASS
...
Lab 77b Task 1
What I practiced: Ansible equivalent for Restrict Service Access by User List
...
## Task 2 - Ansible trap rehearsal for Restrict Service Access by User List
...
ls -ld /run
test -e /run && echo "practice directory exists"
printf 'lab 77b task 2 warm-up at %s
...
Ansible trap rehearsal for Restrict Service Access by User List. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-77b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 77b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 77b Task 2
What I practiced: Ansible trap rehearsal for Restrict Service Access by User List
