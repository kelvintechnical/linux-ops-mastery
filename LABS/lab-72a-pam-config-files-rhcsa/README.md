# Lab 72a - Explore PAM Config Files (RHCSA)
...
- PRACTICE DIRECTORY: `/opt`
- PRIMARY COMMANDS: `/etc/pam.d/`
- TRAPS: T72-A: editing system-auth directly can be overwritten by authselect; T72-B: PAM control flags are order-sensitive and can lock out logins; T41 destroy-restore; T44 cleanup safety

This lab's practice directory is: `/opt`.
...
```bash
export LAB_NUM=72
export LAB_SLUG=pam_config_files
...
Practice directory: /opt
Third-party application trees live here. It is useful for vendor-style service examples.
EOF
id "${USER}"
ls -ld "${SANDBOX}" "${USER_HOME}" /opt
echo "Sandbox built by $(whoami) at $(date -Is)"
echo "exit was: $?"
```

## Task 1 - Canonical RHCSA form for Explore PAM Config Files
...
pwd
ls -ld /opt
test -e /opt && echo "practice directory exists"
printf 'lab 72a task 1 warm-up at %s
...
Canonical RHCSA form for Explore PAM Config Files. This task connects `/etc/pam.d/` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ls -l /etc/pam.d | tee ${SANDBOX}/pam-files.txt
sed -n '1,12p' /etc/pam.d/su | tee ${SANDBOX}/su-pam.txt
ls -ld /opt | tee -a "${SANDBOX}/task-1-practice.txt"
test -e /opt && echo "/opt checked again" | tee -a "${SANDBOX}/task-1-practice.txt"
...
Task 1 is the safe, exam-style version of Explore PAM Config Files. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `/etc/pam.d/` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `/etc/pam.d/` |
| Practice directory | `/opt` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T72-A: editing system-auth directly can be overwritten by authselect |
...
printf 'Persistence check 72a task 1: PASS
...
Lab 72a Task 1
What I practiced: Canonical RHCSA form for Explore PAM Config Files
Trap rehearsed: T72-A: editing system-auth directly can be overwritten by authselect
...
## Task 2 - Trap rehearsal and contrast for Explore PAM Config Files
...
ls -ld /opt
test -e /opt && echo "practice directory exists"
printf 'lab 72a task 2 warm-up at %s
...
Trap rehearsal and contrast for Explore PAM Config Files. This task connects `/etc/pam.d/` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
awk '{print NR ":" $0}' /etc/pam.d/su | tee ${SANDBOX}/control-flags.txt
ls -ld /opt | tee -a "${SANDBOX}/task-2-practice.txt"
test -e /opt && echo "/opt checked again" | tee -a "${SANDBOX}/task-2-practice.txt"
...
| Practice directory | `/opt` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T72-B: PAM control flags are order-sensitive and can lock out logins |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 72a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 72a Task 2
What I practiced: Trap rehearsal and contrast for Explore PAM Config Files
Trap rehearsed: T72-B: PAM control flags are order-sensitive and can lock out logins
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 72 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
