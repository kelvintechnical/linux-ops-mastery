# Lab 81a - Persistent Context Restoration (RHCSA)
...
- PRIMARY COMMANDS: `semanage fcontext, restorecon`
- TRAPS: T81-A: semanage records the rule but restorecon applies it; T81-B: regex path patterns need quotes and correct (/.*)? suffix; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=81
export LAB_SLUG=selinux_fcontext_restorecon
...
## Task 1 - Canonical RHCSA form for Persistent Context Restoration
...
printf 'lab 81a task 1 warm-up at %s
...
Canonical RHCSA form for Persistent Context Restoration. This task connects `semanage fcontext, restorecon` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 81 evidence for Persistent Context Restoration\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: semanage fcontext, restorecon\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Persistent Context Restoration. It creates evidence first and avoids changing production state without a restore path.
...
| `fcontext` | Practice token for this lab | Repeat it until recognition is automatic |
| `restorecon` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `semanage fcontext, restorecon` |
| Practice directory | `/bin` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T81-A: semanage records the rule but restorecon applies it |
...
printf 'Persistence check 81a task 1: PASS
...
Lab 81a Task 1
What I practiced: Canonical RHCSA form for Persistent Context Restoration
Trap rehearsed: T81-A: semanage records the rule but restorecon applies it
...
## Task 2 - Trap rehearsal and contrast for Persistent Context Restoration
...
ls -ld /bin
test -e /bin && echo "practice directory exists"
printf 'lab 81a task 2 warm-up at %s
...
Trap rehearsal and contrast for Persistent Context Restoration. This task connects `semanage fcontext, restorecon` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T81-B: regex path patterns need quotes and correct (/.*)? suffix\n' | tee ${SANDBOX}/task-2-trap.txt
...
| Practice directory | `/bin` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T81-B: regex path patterns need quotes and correct (/.*)? suffix |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 81a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 81a Task 2
What I practiced: Trap rehearsal and contrast for Persistent Context Restoration
Trap rehearsed: T81-B: regex path patterns need quotes and correct (/.*)? suffix
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 81 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
