# Lab 67c - Configure ICMP Filters (Verify)
...
## Task 1 - Audit evidence for Configure ICMP Filters
...
printf 'lab 67c task 1 warm-up at %s
...
Audit evidence for Configure ICMP Filters. This task connects `firewall-cmd --add-icmp-block` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 67c task 1: PASS
...
Lab 67c Task 1
What I practiced: Audit evidence for Configure ICMP Filters
...
## Task 2 - Destroy-restore drill for Configure ICMP Filters
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 67c task 2 warm-up at %s
...
Destroy-restore drill for Configure ICMP Filters. This task connects `firewall-cmd --add-icmp-block` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-67-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-67-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 67c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 67c Task 2
What I practiced: Destroy-restore drill for Configure ICMP Filters
