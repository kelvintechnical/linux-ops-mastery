# Lab 66a - Setup Port Forwarding DNAT (RHCSA)
...
## Task 1 - Canonical RHCSA form for Setup Port Forwarding DNAT
...
printf 'lab 66a task 1 warm-up at %s
...
Canonical RHCSA form for Setup Port Forwarding DNAT. This task connects `firewall-cmd rich rules` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
sudo firewall-cmd --permanent --add-forward-port=port=80:proto=tcp:toport=8008
sudo firewall-cmd --reload
sudo firewall-cmd --list-forward-ports | tee ${SANDBOX}/forward-ports.txt
...
Task 1 is the safe, exam-style version of Setup Port Forwarding DNAT. It creates evidence first and avoids changing production state without a restore path.
...
| `rich` | Practice token for this lab | Repeat it until recognition is automatic |
| `rules` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `firewall-cmd rich rules` |
| Practice directory | `/etc` |
...
printf 'Persistence check 66a task 1: PASS
...
Lab 66a Task 1
What I practiced: Canonical RHCSA form for Setup Port Forwarding DNAT
...
## Task 2 - Trap rehearsal and contrast for Setup Port Forwarding DNAT
...
ls -ld /etc
test -e /etc && echo "practice directory exists"
printf 'lab 66a task 2 warm-up at %s
...
Trap rehearsal and contrast for Setup Port Forwarding DNAT. This task connects `firewall-cmd rich rules` to a repeatable artifact in `${SANDBOX}`.
...
sudo firewall-cmd --reload
sudo firewall-cmd --list-rich-rules | tee ${SANDBOX}/dnat-rich.txt
...
printf 'Persistence check 66a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 66a Task 2
What I practiced: Trap rehearsal and contrast for Setup Port Forwarding DNAT
