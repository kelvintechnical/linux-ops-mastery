# Lab 86c - Enable Verbose Kernel Messages (Verify)
...
## Task 1 - Audit evidence for Enable Verbose Kernel Messages
...
printf 'lab 86c task 1 warm-up at %s
...
Audit evidence for Enable Verbose Kernel Messages. This task connects `GRUB_CMDLINE_LINUX` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 86c task 1: PASS
...
Lab 86c Task 1
What I practiced: Audit evidence for Enable Verbose Kernel Messages
...
## Task 2 - Destroy-restore drill for Enable Verbose Kernel Messages
...
ls -ld /etc
test -e /etc && echo "practice directory exists"
printf 'lab 86c task 2 warm-up at %s
...
Destroy-restore drill for Enable Verbose Kernel Messages. This task connects `GRUB_CMDLINE_LINUX` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-86-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-86-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 86c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 86c Task 2
What I practiced: Destroy-restore drill for Enable Verbose Kernel Messages
