# Lab 63b - Configure IP Masquerading NAT (Ansible)
...
## Task 1 - Ansible equivalent for Configure IP Masquerading NAT
...
printf 'lab 63b task 1 warm-up at %s
...
Ansible equivalent for Configure IP Masquerading NAT. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-63b.yml" <<'EOF'
---
- name: Lab 63b Task 1 - Configure IP Masquerading NAT
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Manage firewalld state safely
      ansible.posix.firewalld:
        ...
        state: enabled
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-63b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 63b task 1: PASS
...
Lab 63b Task 1
What I practiced: Ansible equivalent for Configure IP Masquerading NAT
...
## Task 2 - Ansible trap rehearsal for Configure IP Masquerading NAT
...
ls -ld /lib
test -e /lib && echo "practice directory exists"
printf 'lab 63b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure IP Masquerading NAT. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-63b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 63b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 63b Task 2
What I practiced: Ansible trap rehearsal for Configure IP Masquerading NAT
