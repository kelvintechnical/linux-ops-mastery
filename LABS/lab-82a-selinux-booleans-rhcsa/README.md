# Lab 82a - Toggling SELinux Booleans (RHCSA)
...
- PRIMARY COMMANDS: `getsebool, setsebool -P`
- TRAPS: T82-A: setsebool without -P is runtime-only; T82-B: boolean names must match exactly; grep first; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=82
export LAB_SLUG=selinux_booleans
...
## Task 1 - Canonical RHCSA form for Toggling SELinux Booleans
...
printf 'lab 82a task 1 warm-up at %s
...
Canonical RHCSA form for Toggling SELinux Booleans. This task connects `getsebool, setsebool -P` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 82 evidence for Toggling SELinux Booleans\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: getsebool, setsebool -P\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Toggling SELinux Booleans. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `getsebool` | Practice token for this lab | Repeat it until recognition is automatic |
| `setsebool` | Practice token for this lab | Repeat it until recognition is automatic |
...
| Concept | Answer |
|---|---|
| Command family | `getsebool, setsebool -P` |
| Practice directory | `/sbin` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T82-A: setsebool without -P is runtime-only |
...
printf 'Persistence check 82a task 1: PASS
...
Lab 82a Task 1
What I practiced: Canonical RHCSA form for Toggling SELinux Booleans
Trap rehearsed: T82-A: setsebool without -P is runtime-only
...
## Task 2 - Trap rehearsal and contrast for Toggling SELinux Booleans
...
ls -ld /sbin
test -e /sbin && echo "practice directory exists"
printf 'lab 82a task 2 warm-up at %s
...
Trap rehearsal and contrast for Toggling SELinux Booleans. This task connects `getsebool, setsebool -P` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T82-B: boolean names must match exactly; grep first\n' | tee ${SANDBOX}/task-2-trap.txt
...
| Practice directory | `/sbin` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T82-B: boolean names must match exactly; grep first |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 82a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 82a Task 2
What I practiced: Trap rehearsal and contrast for Toggling SELinux Booleans
Trap rehearsed: T82-B: boolean names must match exactly; grep first
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 82 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
