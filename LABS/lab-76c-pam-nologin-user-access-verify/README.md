# Lab 76c - Use PAM to Limit User Access (Verify)
...
## Task 1 - Audit evidence for Use PAM to Limit User Access
...
printf 'lab 76c task 1 warm-up at %s
...
Audit evidence for Use PAM to Limit User Access. This task connects `/etc/nologin` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 76c task 1: PASS
...
Lab 76c Task 1
What I practiced: Audit evidence for Use PAM to Limit User Access
...
## Task 2 - Destroy-restore drill for Use PAM to Limit User Access
...
ls -ld /sys
test -e /sys && echo "practice directory exists"
printf 'lab 76c task 2 warm-up at %s
...
Destroy-restore drill for Use PAM to Limit User Access. This task connects `/etc/nologin` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-76-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-76-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 76c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 76c Task 2
What I practiced: Destroy-restore drill for Use PAM to Limit User Access
