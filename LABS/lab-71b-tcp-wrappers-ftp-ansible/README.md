# Lab 71b - Configure TCP Wrappers for FTP (Ansible)
...
## Task 1 - Ansible equivalent for Configure TCP Wrappers for FTP
...
printf 'lab 71b task 1 warm-up at %s
...
Ansible equivalent for Configure TCP Wrappers for FTP. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-71b.yml" <<'EOF'
---
- name: Lab 71b Task 1 - Configure TCP Wrappers for FTP
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Write wrapper file from a template copy
      ansible.builtin.copy:
        dest: /tmp/lab-wrapper-example.conf
        content: 'sshd : 127.0.0.1\n'
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-71b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 71b task 1: PASS
...
Lab 71b Task 1
What I practiced: Ansible equivalent for Configure TCP Wrappers for FTP
...
## Task 2 - Ansible trap rehearsal for Configure TCP Wrappers for FTP
...
ls -ld /tmp
test -e /tmp && echo "practice directory exists"
printf 'lab 71b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure TCP Wrappers for FTP. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-71b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 71b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 71b Task 2
What I practiced: Ansible trap rehearsal for Configure TCP Wrappers for FTP
