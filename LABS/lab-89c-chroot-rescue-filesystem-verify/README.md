# Lab 89c - Chroot into Rescue Filesystem (Verify)
...
## Task 1 - Audit evidence for Chroot into Rescue Filesystem
...
printf 'lab 89c task 1 warm-up at %s
...
Audit evidence for Chroot into Rescue Filesystem. This task connects `chroot /mnt/sysimage` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 89c task 1: PASS
...
Lab 89c Task 1
What I practiced: Audit evidence for Chroot into Rescue Filesystem
...
## Task 2 - Destroy-restore drill for Chroot into Rescue Filesystem
...
ls -ld /root
test -e /root && echo "practice directory exists"
printf 'lab 89c task 2 warm-up at %s
...
Destroy-restore drill for Chroot into Rescue Filesystem. This task connects `chroot /mnt/sysimage` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-89-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-89-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 89c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 89c Task 2
What I practiced: Destroy-restore drill for Chroot into Rescue Filesystem
