# Lab 71c - Configure TCP Wrappers for FTP (Verify)
...
## Task 1 - Audit evidence for Configure TCP Wrappers for FTP
...
printf 'lab 71c task 1 warm-up at %s
...
Audit evidence for Configure TCP Wrappers for FTP. This task connects `vsftpd, /etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 71c task 1: PASS
...
Lab 71c Task 1
What I practiced: Audit evidence for Configure TCP Wrappers for FTP
...
## Task 2 - Destroy-restore drill for Configure TCP Wrappers for FTP
...
ls -ld /tmp
test -e /tmp && echo "practice directory exists"
printf 'lab 71c task 2 warm-up at %s
...
Destroy-restore drill for Configure TCP Wrappers for FTP. This task connects `vsftpd, /etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-71-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-71-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 71c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 71c Task 2
What I practiced: Destroy-restore drill for Configure TCP Wrappers for FTP
