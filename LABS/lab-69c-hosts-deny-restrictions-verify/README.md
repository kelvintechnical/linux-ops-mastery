# Lab 69c - Restrict Access via hosts.deny (Verify)
...
## Task 1 - Audit evidence for Restrict Access via hosts.deny
...
printf 'lab 69c task 1 warm-up at %s
...
Audit evidence for Restrict Access via hosts.deny. This task connects `/etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 69c task 1: PASS
...
Lab 69c Task 1
What I practiced: Audit evidence for Restrict Access via hosts.deny
...
## Task 2 - Destroy-restore drill for Restrict Access via hosts.deny
...
ls -ld /root
test -e /root && echo "practice directory exists"
printf 'lab 69c task 2 warm-up at %s
...
Destroy-restore drill for Restrict Access via hosts.deny. This task connects `/etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-69-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-69-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 69c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 69c Task 2
What I practiced: Destroy-restore drill for Restrict Access via hosts.deny
