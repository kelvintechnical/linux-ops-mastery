# Lab 75b - Configure PAM to Limit root Access (Ansible)
...
## Task 1 - Ansible equivalent for Configure PAM to Limit root Access
...
printf 'lab 75b task 1 warm-up at %s
...
Ansible equivalent for Configure PAM to Limit root Access. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-75b.yml" <<'EOF'
---
- name: Lab 75b Task 1 - Configure PAM to Limit root Access
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Stage PAM example safely
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-75b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 75b task 1: PASS
...
Lab 75b Task 1
What I practiced: Ansible equivalent for Configure PAM to Limit root Access
...
## Task 2 - Ansible trap rehearsal for Configure PAM to Limit root Access
...
ls -ld /proc
test -e /proc && echo "practice directory exists"
printf 'lab 75b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure PAM to Limit root Access. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-75b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 75b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 75b Task 2
What I practiced: Ansible trap rehearsal for Configure PAM to Limit root Access
