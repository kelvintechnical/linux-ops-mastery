# Lab 74c - Implement Password Complexity (Verify)
...
## Task 1 - Audit evidence for Implement Password Complexity
...
printf 'lab 74c task 1 warm-up at %s
...
Audit evidence for Implement Password Complexity. This task connects `pam_pwquality.so, system-auth` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 74c task 1: PASS
...
Lab 74c Task 1
What I practiced: Audit evidence for Implement Password Complexity
...
## Task 2 - Destroy-restore drill for Implement Password Complexity
...
ls -ld /dev
test -e /dev && echo "practice directory exists"
printf 'lab 74c task 2 warm-up at %s
...
Destroy-restore drill for Implement Password Complexity. This task connects `pam_pwquality.so, system-auth` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-74-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-74-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 74c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 74c Task 2
What I practiced: Destroy-restore drill for Implement Password Complexity
