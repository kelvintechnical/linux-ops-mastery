# Lab 83c - SELinux User Mapping (Verify)
...
## Task 1 - Audit evidence for SELinux User Mapping
...
printf 'lab 83c task 1 warm-up at %s
...
Audit evidence for SELinux User Mapping. This task connects `semanage login` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 83c task 1: PASS
...
Lab 83c Task 1
What I practiced: Audit evidence for SELinux User Mapping
...
## Task 2 - Destroy-restore drill for SELinux User Mapping
...
ls -ld /lib
test -e /lib && echo "practice directory exists"
printf 'lab 83c task 2 warm-up at %s
...
Destroy-restore drill for SELinux User Mapping. This task connects `semanage login` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-83-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-83-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 83c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 83c Task 2
What I practiced: Destroy-restore drill for SELinux User Mapping
