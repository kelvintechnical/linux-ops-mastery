# Lab 61c - Reassign Interfaces to Zones (Verify)
...
## Task 1 - Audit evidence for Reassign Interfaces to Zones
...
printf 'lab 61c task 1 warm-up at %s
...
Audit evidence for Reassign Interfaces to Zones. This task connects `firewall-cmd --change-interface` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 61c task 1: PASS
...
Lab 61c Task 1
What I practiced: Audit evidence for Reassign Interfaces to Zones
...
## Task 2 - Destroy-restore drill for Reassign Interfaces to Zones
...
ls -ld /bin
test -e /bin && echo "practice directory exists"
printf 'lab 61c task 2 warm-up at %s
...
Destroy-restore drill for Reassign Interfaces to Zones. This task connects `firewall-cmd --change-interface` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-61-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-61-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 61c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 61c Task 2
What I practiced: Destroy-restore drill for Reassign Interfaces to Zones
