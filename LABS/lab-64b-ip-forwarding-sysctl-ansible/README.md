# Lab 64b - Configure IP Forwarding (Ansible)
...
## Task 1 - Ansible equivalent for Configure IP Forwarding
...
printf 'lab 64b task 1 warm-up at %s
...
Ansible equivalent for Configure IP Forwarding. This task connects `ansible.posix.sysctl` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-64b.yml" <<'EOF'
---
- name: Lab 64b Task 1 - Configure IP Forwarding
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Persist IPv4 forwarding
      ansible.posix.sysctl:
        ...
        reload: true
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-64b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
The Ansible tier uses `ansible.posix.sysctl` or an honest boundary pattern when no safe dedicated module exists.
...
|---|---|---|
| `ansible.posix.sysctl` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `ansible.posix.sysctl` |
| Practice directory | `/lib64` |
...
printf 'Persistence check 64b task 1: PASS
...
Lab 64b Task 1
What I practiced: Ansible equivalent for Configure IP Forwarding
...
## Task 2 - Ansible trap rehearsal for Configure IP Forwarding
...
ls -ld /lib64
test -e /lib64 && echo "practice directory exists"
printf 'lab 64b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure IP Forwarding. This task connects `ansible.posix.sysctl` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-64b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 64b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 64b Task 2
What I practiced: Ansible trap rehearsal for Configure IP Forwarding
