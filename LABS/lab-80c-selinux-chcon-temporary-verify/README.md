# Lab 80c - Temporary Context Changes (Verify)
...
## Task 1 - Audit evidence for Temporary Context Changes
...
printf 'lab 80c task 1 warm-up at %s
...
Audit evidence for Temporary Context Changes. This task connects `chcon` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 80c task 1: PASS
...
Lab 80c Task 1
What I practiced: Audit evidence for Temporary Context Changes
...
## Task 2 - Destroy-restore drill for Temporary Context Changes
...
ls -ld /afs
test -e /afs && echo "practice directory exists"
printf 'lab 80c task 2 warm-up at %s
...
Destroy-restore drill for Temporary Context Changes. This task connects `chcon` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-80-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-80-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 80c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 80c Task 2
What I practiced: Destroy-restore drill for Temporary Context Changes
