# Lab 65c - Configure Rich Rules (Verify)
...
## Task 1 - Audit evidence for Configure Rich Rules
...
printf 'lab 65c task 1 warm-up at %s
...
Audit evidence for Configure Rich Rules. This task connects `firewall-cmd --add-rich-rule` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 65c task 1: PASS
...
Lab 65c Task 1
What I practiced: Audit evidence for Configure Rich Rules
...
## Task 2 - Destroy-restore drill for Configure Rich Rules
...
ls -ld /usr
test -e /usr && echo "practice directory exists"
printf 'lab 65c task 2 warm-up at %s
...
Destroy-restore drill for Configure Rich Rules. This task connects `firewall-cmd --add-rich-rule` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-65-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-65-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 65c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 65c Task 2
What I practiced: Destroy-restore drill for Configure Rich Rules
