# Lab 70b - Allow Specific Access via hosts.allow (Ansible)
...
## Task 1 - Ansible equivalent for Allow Specific Access via hosts.allow
...
printf 'lab 70b task 1 warm-up at %s
...
Ansible equivalent for Allow Specific Access via hosts.allow. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-70b.yml" <<'EOF'
---
- name: Lab 70b Task 1 - Allow Specific Access via hosts.allow
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Write wrapper file from a template copy
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-70b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
| Trap risk | T70-A: hosts.allow is checked before hosts.deny, so order across files matters |
...
printf 'Persistence check 70b task 1: PASS
...
Lab 70b Task 1
What I practiced: Ansible equivalent for Allow Specific Access via hosts.allow
Trap rehearsed: T70-A: hosts.allow is checked before hosts.deny, so order across files matters
...
## Task 2 - Ansible trap rehearsal for Allow Specific Access via hosts.allow
...
ls -ld /var
test -e /var && echo "practice directory exists"
printf 'lab 70b task 2 warm-up at %s
...
Ansible trap rehearsal for Allow Specific Access via hosts.allow. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-70b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
| Practice directory | `/var` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T70-B: broad ALL : ALL in hosts.allow defeats the deny file |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 70b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 70b Task 2
What I practiced: Ansible trap rehearsal for Allow Specific Access via hosts.allow
Trap rehearsed: T70-B: broad ALL : ALL in hosts.allow defeats the deny file
What I will verify later: evidence file exists and the system state is restored.
EOF
```
