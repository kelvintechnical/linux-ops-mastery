# Lab 65b - Configure Rich Rules (Ansible)
...
## Task 1 - Ansible equivalent for Configure Rich Rules
...
printf 'lab 65b task 1 warm-up at %s
...
Ansible equivalent for Configure Rich Rules. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-65b.yml" <<'EOF'
---
- name: Lab 65b Task 1 - Configure Rich Rules
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Manage firewalld state safely
      ansible.posix.firewalld:
        ...
        state: enabled
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-65b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 65b task 1: PASS
...
Lab 65b Task 1
What I practiced: Ansible equivalent for Configure Rich Rules
...
## Task 2 - Ansible trap rehearsal for Configure Rich Rules
...
ls -ld /usr
test -e /usr && echo "practice directory exists"
printf 'lab 65b task 2 warm-up at %s
...
Ansible trap rehearsal for Configure Rich Rules. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-65b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 65b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 65b Task 2
What I practiced: Ansible trap rehearsal for Configure Rich Rules
