# Lab 62b - Allow Services Through Firewall (Ansible)
...
## Task 1 - Ansible equivalent for Allow Services Through Firewall
...
printf 'lab 62b task 1 warm-up at %s
...
Ansible equivalent for Allow Services Through Firewall. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-62b.yml" <<'EOF'
---
- name: Lab 62b Task 1 - Allow Services Through Firewall
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Manage firewalld state safely
      ansible.posix.firewalld:
        ...
        state: enabled
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-62b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 62b task 1: PASS
...
Lab 62b Task 1
What I practiced: Ansible equivalent for Allow Services Through Firewall
...
## Task 2 - Ansible trap rehearsal for Allow Services Through Firewall
...
ls -ld /sbin
test -e /sbin && echo "practice directory exists"
printf 'lab 62b task 2 warm-up at %s
...
Ansible trap rehearsal for Allow Services Through Firewall. This task connects `ansible.posix.firewalld` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-62b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 62b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 62b Task 2
What I practiced: Ansible trap rehearsal for Allow Services Through Firewall
