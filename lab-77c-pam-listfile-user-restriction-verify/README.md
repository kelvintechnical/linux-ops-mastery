# Lab 77c - Restrict Service Access by User List (Verify)
...
## Task 1 - Audit evidence for Restrict Service Access by User List
...
printf 'lab 77c task 1 warm-up at %s
...
Audit evidence for Restrict Service Access by User List. This task connects `pam_listfile.so` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 77c task 1: PASS
...
Lab 77c Task 1
What I practiced: Audit evidence for Restrict Service Access by User List
...
## Task 2 - Destroy-restore drill for Restrict Service Access by User List
...
ls -ld /run
test -e /run && echo "practice directory exists"
printf 'lab 77c task 2 warm-up at %s
...
Destroy-restore drill for Restrict Service Access by User List. This task connects `pam_listfile.so` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-77-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-77-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 77c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 77c Task 2
What I practiced: Destroy-restore drill for Restrict Service Access by User List
