# Lab 81c - Persistent Context Restoration (Verify)
...
## Task 1 - Audit evidence for Persistent Context Restoration
...
printf 'lab 81c task 1 warm-up at %s
...
Audit evidence for Persistent Context Restoration. This task connects `semanage fcontext, restorecon` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 81c task 1: PASS
...
Lab 81c Task 1
What I practiced: Audit evidence for Persistent Context Restoration
...
## Task 2 - Destroy-restore drill for Persistent Context Restoration
...
ls -ld /bin
test -e /bin && echo "practice directory exists"
printf 'lab 81c task 2 warm-up at %s
...
Destroy-restore drill for Persistent Context Restoration. This task connects `semanage fcontext, restorecon` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-81-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-81-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 81c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 81c Task 2
What I practiced: Destroy-restore drill for Persistent Context Restoration
