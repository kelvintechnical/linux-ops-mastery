# Lab 72c - Explore PAM Config Files (Verify)
...
## Task 1 - Audit evidence for Explore PAM Config Files
...
printf 'lab 72c task 1 warm-up at %s
...
Audit evidence for Explore PAM Config Files. This task connects `/etc/pam.d/` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 72c task 1: PASS
...
Lab 72c Task 1
What I practiced: Audit evidence for Explore PAM Config Files
...
## Task 2 - Destroy-restore drill for Explore PAM Config Files
...
ls -ld /opt
test -e /opt && echo "practice directory exists"
printf 'lab 72c task 2 warm-up at %s
...
Destroy-restore drill for Explore PAM Config Files. This task connects `/etc/pam.d/` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-72-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-72-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 72c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 72c Task 2
What I practiced: Destroy-restore drill for Explore PAM Config Files
