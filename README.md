...
| # | Lab | Key Commands |
|---|-----|-------------|
| 00 | Ansible Control Node Trilogy: [00a RHCSA](lab-00a-ansible-control-node-rhcsa/) · [00b Ansible](lab-00b-ansible-control-node-ansible/) · [00c Verify](lab-00c-ansible-control-node-verify/) | `dnf install ansible-core`, `ansible-galaxy collection install`, `~/.ansible.cfg`, `~/inventory`, `ansible -m ping`, `ansible-playbook --check --diff` — **prerequisite for all b-labs**. 2-task trilogy: install + collections + config, first FQCN playbook, audit + T41 restore drill. Traps T00-A/B/C. |
...
| 08 | Copying Files and Directories Trilogy: [08a RHCSA](lab-08a-copying-files-rhcsa/) · [08b Ansible](lab-08b-copying-files-ansible/) · [08c Verify](lab-08c-copying-files-verify/) | `cp`, `cp -R`, `cp -a`, `cp --preserve=mode,timestamps,context`, `--no-preserve=ownership` + **Ansible** `ansible.builtin.copy` with `remote_src:`, `mode:`, `owner:`, `preserve:`, `backup:` — 2-task trilogy on `/etc/skel`. Traps T08-A/B/C. |
