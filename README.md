...
| # | Lab | Key Commands |
|---|-----|-------------|
| 00 | [Ansible Control Node Setup](ansible-control-node-setup/) | `dnf install ansible-core`, `ansible-galaxy collection install`, `~/.ansible.cfg`, `~/inventory`, `ansible -m ping`, `ansible-playbook --check --diff` — **prerequisite for Task 4 of every other lab**. 5-task 3-1-1: install + collections + config + first playbook + verification capstone. Traps T00-A/B/C. |
| 05 | [Directory Navigation](directory-nav/) | `pwd -L/-P`, `cd ..`, `cd ~`, `cd -`, `$OLDPWD` + **Ansible Boundary** (no `cd` module; `chdir:` only) — 5-task 3-1-1 ADHD drill on `/usr`. Traps T41 + T42 + T43. |
| 06 | [Listing Files and SELinux Contexts](listing-files-selinux/) | `ls -l/-lZ/-dZ`, `matchpathcon`, `semanage fcontext`, `restorecon -Rv` + **Ansible** `community.general.sefcontext` (RHCE permanent fcontext) — 5-task 3-1-1 ADHD drill on `/etc`. Traps T01 + T02 + T03. |
| 07 | [Creating Empty Files and Timestamps](touch-timestamps/) | `touch`, `touch -t/-d/-r/-a/-m`, `stat -c %x/%y/%z`, `find -mtime/-mmin/-newer` + **Ansible** `ansible.builtin.file: state=touch` with `modification_time:` / `access_time:` — 5-task 3-1-1 on `/var/log`. Traps T07 + T08. |
| 08 | [Copying Files and Directories](copying-files-directories/) | `cp`, `cp -R`, `cp -a`, `cp --preserve=mode,timestamps,context`, `--no-preserve=ownership` + **Ansible** `ansible.builtin.copy` with `remote_src:`, `mode:`, `owner:`, `preserve:`, `backup:` — 5-task 3-1-1 on `/etc/skel`. Traps T08-A/B/C. |
| 09 | [Hard and Soft Links](hard-and-soft-links/) | `ln`, `ln -s`, `readlink -f`, `find -inum`, `stat -c %h`, dangling symlinks (`test -L` vs `test -e`) + **Ansible** `ansible.builtin.file: state=link` / `state=hard` with `force: true` — 5-task 3-1-1 on `/var/log`. Traps T17 + T18 + T19. |
| 10 | [Moving and Renaming Files](moving-renaming-files/) | `mv`, `mv -i/-n/-b/-u/-t`, same-fs atomic rename vs cross-fs `cp+rm`, hard-link survival + **Ansible** `ansible.builtin.command: mv` with `creates:`/`removes:` (Boundary) AND `ansible.builtin.copy` for atomic config replace with `backup: true` — 5-task 3-1-1 on `/var`. Traps T10-A/B/C. |
