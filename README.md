...
| # | Lab | Key Commands |
|---|-----|-------------|
| 00 | Ansible Control Node Setup Trilogy: [00a RHCSA](lab-00a-ansible-control-node-setup-rhcsa/) · [00b Ansible](lab-00b-ansible-control-node-setup-ansible/) · [00c Verify](lab-00c-ansible-control-node-setup-verify/) | `dnf install ansible-core`, `ansible-galaxy collection install`, `~/.ansible.cfg`, `~/inventory`, `ansible -m ping`, `ansible-playbook --check --diff` — **prerequisite for all b-labs**. 2-task trilogy: install + collections + config, first FQCN playbook, audit + T41 restore drill. Traps T00-A/B/C. |
| 05 | Directory Navigation Trilogy: [05a RHCSA](lab-05a-directory-navigation-rhcsa/) · [05b Ansible Boundary](lab-05b-directory-navigation-ansible/) · [05c Verify](lab-05c-directory-navigation-verify/) | `pwd -L/-P`, `cd ..`, `cd ~`, `cd -`, `$OLDPWD` + **Ansible Boundary** (no `cd` module; `chdir:` only) — 2-task trilogy on `/usr`. Traps T41 + T42 + T43. |
| 06 | Listing Files and SELinux Trilogy: [06a RHCSA](lab-06a-listing-files-selinux-rhcsa/) · [06b Ansible](lab-06b-listing-files-selinux-ansible/) · [06c Verify](lab-06c-listing-files-selinux-verify/) | `ls -l/-lZ/-dZ`, `matchpathcon`, `semanage fcontext`, `restorecon -Rv` + **Ansible** `community.general.sefcontext` — 2-task trilogy on `/etc`. Traps T01 + T02 + T03. |
| 07 | Touch Timestamps Trilogy: [07a RHCSA](lab-07a-touch-timestamps-rhcsa/) · [07b Ansible](lab-07b-touch-timestamps-ansible/) · [07c Verify](lab-07c-touch-timestamps-verify/) | `touch`, `touch -t/-d/-r/-a/-m`, `stat -c %x/%y/%z`, `find -mtime/-mmin/-newer` + **Ansible** `ansible.builtin.file: state=touch` with `modification_time:` / `access_time:` — 2-task trilogy on `/var/log`. Traps T07 + T08. |
| 08 | Copying Files and Directories Trilogy: [08a RHCSA](lab-08a-copying-files-directories-rhcsa/) · [08b Ansible](lab-08b-copying-files-directories-ansible/) · [08c Verify](lab-08c-copying-files-directories-verify/) | `cp`, `cp -R`, `cp -a`, `cp --preserve=mode,timestamps,context`, `--no-preserve=ownership` + **Ansible** `ansible.builtin.copy` with `remote_src:`, `mode:`, `owner:`, `preserve:`, `backup:` — 2-task trilogy on `/etc/skel`. Traps T08-A/B/C. |
| 09 | Hard and Soft Links Trilogy: [09a RHCSA](lab-09a-hard-and-soft-links-rhcsa/) · [09b Ansible](lab-09b-hard-and-soft-links-ansible/) · [09c Verify](lab-09c-hard-and-soft-links-verify/) | `ln`, `ln -s`, `readlink -f`, `find -inum`, `stat -c %h`, dangling symlinks (`test -L` vs `test -e`) + **Ansible** `ansible.builtin.file: state=link` / `state=hard` with `force: true` — 2-task trilogy on `/var/log`. Traps T17 + T18 + T19. |
| 10 | Moving and Renaming Files Trilogy: [10a RHCSA](lab-10a-moving-renaming-files-rhcsa/) · [10b Ansible](lab-10b-moving-renaming-files-ansible/) · [10c Verify](lab-10c-moving-renaming-files-verify/) | `mv`, `mv -i/-n/-b/-u/-t`, same-fs atomic rename vs cross-fs `cp+rm`, hard-link survival + **Ansible Boundary** `command: mv` with `creates:`/`removes:` and `copy` with `backup: true` — 2-task trilogy on `/var`. Traps T10-A/B/C. |
| 11 | Safe Deletion Trilogy: [11a RHCSA](lab-11a-removing-files-rhcsa/) · [11b Ansible](lab-11b-removing-files-ansible/) · [11c Verify](lab-11c-removing-files-verify/) | `rm`, `rmdir`, `rm -rf` + **Ansible** `ansible.builtin.file: state=absent` — 2-task trilogy for safe deletion, idempotent removal, and audit/persistence verification. Traps T11-A/B/C/D/E. |
| 12 | Nested Directories Trilogy: [12a RHCSA](lab-12a-creating-nested-directories-rhcsa/) · [12b Ansible](lab-12b-creating-nested-directories-ansible/) · [12c Verify](lab-12c-creating-nested-directories-verify/) | `mkdir`, `mkdir -p`, `mkdir -m`, brace expansion + **Ansible** `ansible.builtin.file: state=directory` — 2-task trilogy for directory trees, mode control, idempotence, and audit. Traps T12-A/B/C/D/E. |
