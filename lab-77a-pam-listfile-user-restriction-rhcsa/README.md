# Lab 77a - Restrict Service Access by User List (RHCSA)
...
- PRIMARY COMMANDS: `pam_listfile.so`
- TRAPS: T77-A: item=user versus item=group changes what the list means; T77-B: onerr=succeed can silently allow access when the list file is missing; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=77
export LAB_SLUG=pam_listfile_user_restriction
...
Practice directory: /run
Runtime state, PID files, sockets, and locks live here. It is rebuilt on each boot.
EOF
id "${USER}"
ls -ld "${SANDBOX}" "${USER_HOME}" /run
echo "Sandbox built by $(whoami) at $(date -Is)"
echo "exit was: $?"
```

## Task 1 - Canonical RHCSA form for Restrict Service Access by User List
...
pwd
ls -ld /run
test -e /run && echo "practice directory exists"
printf 'lab 77a task 1 warm-up at %s
...
Canonical RHCSA form for Restrict Service Access by User List. This task connects `pam_listfile.so` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 77 evidence for Restrict Service Access by User List\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: pam_listfile.so\n' | tee -a ${SANDBOX}/task-1-topic.txt
ls -ld /run | tee -a "${SANDBOX}/task-1-practice.txt"
test -e /run && echo "/run checked again" | tee -a "${SANDBOX}/task-1-practice.txt"
...
Task 1 is the safe, exam-style version of Restrict Service Access by User List. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `pam_listfile.so` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `pam_listfile.so` |
| Practice directory | `/run` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T77-A: item=user versus item=group changes what the list means |
...
printf 'Persistence check 77a task 1: PASS
...
Lab 77a Task 1
What I practiced: Canonical RHCSA form for Restrict Service Access by User List
Trap rehearsed: T77-A: item=user versus item=group changes what the list means
...
## Task 2 - Trap rehearsal and contrast for Restrict Service Access by User List
...
ls -ld /run
test -e /run && echo "practice directory exists"
printf 'lab 77a task 2 warm-up at %s
...
Trap rehearsal and contrast for Restrict Service Access by User List. This task connects `pam_listfile.so` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T77-B: onerr=succeed can silently allow access when the list file is missing\n' | tee ${SANDBOX}/task-2-trap.txt
printf 'Restore note: verify before cleanup\n' | tee -a ${SANDBOX}/task-2-trap.txt
ls -ld /run | tee -a "${SANDBOX}/task-2-practice.txt"
test -e /run && echo "/run checked again" | tee -a "${SANDBOX}/task-2-practice.txt"
...
| Practice directory | `/run` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T77-B: onerr=succeed can silently allow access when the list file is missing |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 77a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 77a Task 2
What I practiced: Trap rehearsal and contrast for Restrict Service Access by User List
Trap rehearsed: T77-B: onerr=succeed can silently allow access when the list file is missing
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 77 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
