# Lab 07a: Touch + Timestamps (RHCSA) — `touch`, `stat`, `find -mtime/-mmin/-newer`

- **Series:** linux-ops-mastery — File Operations & Shell Fundamentals
- **Trilogy:** `07a` (RHCSA) -> `07b` (Ansible) -> `07c` (Verify)
- **Prerequisite:** Lab 06 trilogy complete
- **Time Estimate:** 30-40 minutes
- **Tasks:** 2 (ADHD format: shell fundamentals + RHCSA capstone)
- **Practice Directory (rotation #07):** `/var/log` (read-only) + `/tmp/touch-lab` (writes)
- **Traps rehearsed this lab:** **T07** (misreading `%z` ctime as "last content edit") and **T08** (bad `touch -t` format)

> **This lab's practice directory is `/var/log`**. Read from `/var/log`; write in `/tmp/touch-lab`.
...
echo ""
echo "/var/log newest five entries:"
ls -lt /var/log 2>/dev/null | head -n 6
```

> **STOP - paste the header output before setup.**

---

## Objective

Use `touch` and `stat` precisely, then search by file age with `find`. By the end you should confidently answer:

1. Which operation changed atime vs mtime vs ctime?
2. How do `touch -t`, `-d`, `-r`, `-a`, and `-m` differ?
3. How do you locate files newer/older than a threshold with `find`?
...
```bash
sudo -i
mkdir -p /tmp/touch-lab /root/rhcsa_journal/lab-07a/task1 /root/rhcsa_journal/lab-07a/task2
cd /tmp/touch-lab
echo "reference" > ref.txt
touch alpha.txt beta.txt gamma.txt
stat -c 'ref mtime=%y' /tmp/touch-lab/ref.txt
echo "exit was: $?"
```

---

## Task 1 — Build timestamp reflexes with `touch` + `stat`

**Practice directory:** `/var/log` (context) and `/tmp/touch-lab` (actions)

### Warm-Up

```bash
ls -l /var/log | head -n 5
stat -c '/var/log/messages mtime=%y' /var/log/messages 2>/dev/null || true
date -Is
echo "exit was: $?"
```
...
Create files, then inspect atime (`%x`), mtime (`%y`), and ctime (`%z`). Prove the difference between data writes and metadata changes.
...
touch task1-a.txt task1-b.txt
stat -c 'A atime=%x mtime=%y ctime=%z' task1-a.txt | tee /root/rhcsa_journal/lab-07a/task1/op.txt

sleep 1
touch task1-a.txt
stat -c 'after touch mtime=%y ctime=%z' task1-a.txt | tee -a /root/rhcsa_journal/lab-07a/task1/op.txt

sleep 1
chmod 600 task1-a.txt
stat -c 'after chmod mtime=%y ctime=%z' task1-a.txt | tee -a /root/rhcsa_journal/lab-07a/task1/op.txt

sleep 1
cat task1-a.txt > /dev/null
stat -c 'after cat atime=%x mtime=%y ctime=%z' task1-a.txt | tee -a /root/rhcsa_journal/lab-07a/task1/op.txt
...
|---|---|
| `%x` | atime - last read |
| `%y` | mtime - last content write |
| `%z` | ctime - inode metadata change |
| **T07** | ctime is **change time**, not creation time |

### Journal write

```bash
cat > /root/rhcsa_journal/lab-07a/task1/done.txt <<EOF
LAB: lab-07a
TASK: task1
DATE: $(date -Is)
STATUS: COMPLETE
...
> **STOP - confirm `op.txt` shows `%x`, `%y`, and `%z` before Task 2.**

---

## Task 2 — Set precise times and search by age

**Practice directory:** `/var/log` + `/tmp/touch-lab`

### Warm-Up

```bash
ls -lt /var/log | head -n 3
stat -c 'ref atime=%x mtime=%y' /tmp/touch-lab/ref.txt
echo "exit was: $?"
```

### Purpose

Practice explicit timestamp assignment (`-t`, `-d`, `-r`, `-a`, `-m`) and age search predicates (`-mtime`, `-mmin`, `-newer`).
...
# T08 guard: full format is [[CC]YY]MMDDhhmm[.ss]
touch -t 202401151230.00 t08-format.txt
touch -d '2025-01-01 08:15:00' t-d.txt
touch -r ref.txt t-r.txt
touch -a -d '2020-02-02 02:02:00' t08-format.txt
touch -m -d '2030-03-03 03:03:00' t08-format.txt

stat -c 't08 atime=%x mtime=%y' t08-format.txt | tee /root/rhcsa_journal/lab-07a/task2/op.txt
stat -c 't-d mtime=%y' t-d.txt | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt
stat -c 'ref=%y t-r=%y' ref.txt t-r.txt | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt

touch -d '8 days ago' older-8d.log
touch -d '40 minutes ago' older-40m.log
touch fresh-now.log

find /tmp/touch-lab -type f -mtime +7 | sort | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt
find /tmp/touch-lab -type f -mmin -60 | sort | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt
find /tmp/touch-lab -type f -newer older-8d.log | sort | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt
find /var/log -maxdepth 1 -type f -mtime -1 2>/dev/null | head -n 5 | tee -a /root/rhcsa_journal/lab-07a/task2/op.txt
...
```bash
cat > /root/rhcsa_journal/lab-07a/task2/done.txt <<EOF
LAB: lab-07a
TASK: task2
DATE: $(date -Is)
STATUS: COMPLETE
...
```bash
# Keep /tmp/touch-lab for 07b and 07c.
ls -1 /tmp/touch-lab | head -n 10
```
...
- [ ] Task 1 - Captured `%x`, `%y`, `%z` and demonstrated `touch` vs `chmod`
- [ ] Task 2 - Used `touch -t/-d/-r/-a/-m` and `find -mtime/-mmin/-newer`
- [ ] Journal files exist under `/root/rhcsa_journal/lab-07a/task1` and `task2`
