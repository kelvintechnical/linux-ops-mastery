# Lab 87a - Generate New GRUB Config (RHCSA)
...
- PRACTICE DIRECTORY: `/boot`
- PRIMARY COMMANDS: `grub2-mkconfig -o /boot/grub2/grub.cfg`
- TRAPS: T87-A: BIOS and UEFI use different output locations; T87-B: writing grub.cfg to the wrong path leaves the system unchanged; T41 destroy-restore; T44 cleanup safety

This lab's practice directory is: `/boot`.
...
Practice directory: /boot
Kernel, initramfs, and GRUB bootloader files live here. Boot labs must treat /boot as high-risk.
EOF
id "${USER}"
...
## Task 1 - Canonical RHCSA form for Generate New GRUB Config
...
pwd
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 87a task 1 warm-up at %s
...
Canonical RHCSA form for Generate New GRUB Config. This task connects `grub2-mkconfig -o /boot/grub2/grub.cfg` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 87 evidence for Generate New GRUB Config\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: grub2-mkconfig -o /boot/grub2/grub.cfg\n' | tee -a ${SANDBOX}/task-1-topic.txt
ls -ld /boot | tee -a "${SANDBOX}/task-1-practice.txt"
test -e /boot && echo "/boot checked again" | tee -a "${SANDBOX}/task-1-practice.txt"
...
Task 1 is the safe, exam-style version of Generate New GRUB Config. It creates evidence first and avoids changing production state without a restore path.
...
| `-o` | Practice token for this lab | Repeat it until recognition is automatic |
| `/boot/grub2/grub.cfg` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `grub2-mkconfig -o /boot/grub2/grub.cfg` |
| Practice directory | `/boot` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T87-A: BIOS and UEFI use different output locations |
...
printf 'Persistence check 87a task 1: PASS
...
Lab 87a Task 1
What I practiced: Canonical RHCSA form for Generate New GRUB Config
Trap rehearsed: T87-A: BIOS and UEFI use different output locations
...
## Task 2 - Trap rehearsal and contrast for Generate New GRUB Config
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 87a task 2 warm-up at %s
...
Trap rehearsal and contrast for Generate New GRUB Config. This task connects `grub2-mkconfig -o /boot/grub2/grub.cfg` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T87-B: writing grub.cfg to the wrong path leaves the system unchanged\n' | tee ${SANDBOX}/task-2-trap.txt
printf 'Restore note: verify before cleanup\n' | tee -a ${SANDBOX}/task-2-trap.txt
ls -ld /boot | tee -a "${SANDBOX}/task-2-practice.txt"
test -e /boot && echo "/boot checked again" | tee -a "${SANDBOX}/task-2-practice.txt"
...
| Practice directory | `/boot` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T87-B: writing grub.cfg to the wrong path leaves the system unchanged |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 87a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 87a Task 2
What I practiced: Trap rehearsal and contrast for Generate New GRUB Config
Trap rehearsed: T87-B: writing grub.cfg to the wrong path leaves the system unchanged
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 87 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
