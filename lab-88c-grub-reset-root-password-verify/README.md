# Lab 88c - Reset Root Password via Boot (Verify)
...
## Task 1 - Audit evidence for Reset Root Password via Boot
...
printf 'lab 88c task 1 warm-up at %s
...
Audit evidence for Reset Root Password via Boot. This task connects `rd.break, chroot, passwd` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 88c task 1: PASS
...
Lab 88c Task 1
What I practiced: Audit evidence for Reset Root Password via Boot
...
## Task 2 - Destroy-restore drill for Reset Root Password via Boot
...
ls -ld /home
test -e /home && echo "practice directory exists"
printf 'lab 88c task 2 warm-up at %s
...
Destroy-restore drill for Reset Root Password via Boot. This task connects `rd.break, chroot, passwd` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-88-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-88-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 88c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 88c Task 2
What I practiced: Destroy-restore drill for Reset Root Password via Boot
