# Lab 74b - Implement Password Complexity (Ansible)
...
## Task 1 - Ansible equivalent for Implement Password Complexity
...
printf 'lab 74b task 1 warm-up at %s
...
Ansible equivalent for Implement Password Complexity. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-74b.yml" <<'EOF'
---
- name: Lab 74b Task 1 - Implement Password Complexity
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Stage PAM example safely
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-74b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 74b task 1: PASS
...
Lab 74b Task 1
What I practiced: Ansible equivalent for Implement Password Complexity
...
## Task 2 - Ansible trap rehearsal for Implement Password Complexity
...
ls -ld /dev
test -e /dev && echo "practice directory exists"
printf 'lab 74b task 2 warm-up at %s
...
Ansible trap rehearsal for Implement Password Complexity. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-74b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 74b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 74b Task 2
What I practiced: Ansible trap rehearsal for Implement Password Complexity
