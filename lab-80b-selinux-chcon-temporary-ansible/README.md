# Lab 80b - Temporary Context Changes (Ansible)
...
## Task 1 - Ansible equivalent for Temporary Context Changes
...
printf 'lab 80b task 1 warm-up at %s
...
Ansible equivalent for Temporary Context Changes. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-80b.yml" <<'EOF'
---
- name: Lab 80b Task 1 - Temporary Context Changes
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Query SELinux state
      ...
      changed_when: false
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-80b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 80b task 1: PASS
...
Lab 80b Task 1
What I practiced: Ansible equivalent for Temporary Context Changes
...
## Task 2 - Ansible trap rehearsal for Temporary Context Changes
...
ls -ld /afs
test -e /afs && echo "practice directory exists"
printf 'lab 80b task 2 warm-up at %s
...
Ansible trap rehearsal for Temporary Context Changes. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-80b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 80b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 80b Task 2
What I practiced: Ansible trap rehearsal for Temporary Context Changes
