# Lab 84c - Troubleshooting SELinux (Verify)
...
## Task 1 - Audit evidence for Troubleshooting SELinux
...
printf 'lab 84c task 1 warm-up at %s
...
Audit evidence for Troubleshooting SELinux. This task connects `audit.log, sealert` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 84c task 1: PASS
...
Lab 84c Task 1
What I practiced: Audit evidence for Troubleshooting SELinux
...
## Task 2 - Destroy-restore drill for Troubleshooting SELinux
...
ls -ld /lib64
test -e /lib64 && echo "practice directory exists"
printf 'lab 84c task 2 warm-up at %s
...
Destroy-restore drill for Troubleshooting SELinux. This task connects `audit.log, sealert` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-84-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-84-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 84c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 84c Task 2
What I practiced: Destroy-restore drill for Troubleshooting SELinux
