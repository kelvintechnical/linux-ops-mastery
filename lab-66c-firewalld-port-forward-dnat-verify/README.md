# Lab 66c - Setup Port Forwarding DNAT (Verify)
...
## Task 1 - Audit evidence for Setup Port Forwarding DNAT
...
printf 'lab 66c task 1 warm-up at %s
...
Audit evidence for Setup Port Forwarding DNAT. This task connects `firewall-cmd rich rules` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 66c task 1: PASS
...
Lab 66c Task 1
What I practiced: Audit evidence for Setup Port Forwarding DNAT
...
## Task 2 - Destroy-restore drill for Setup Port Forwarding DNAT
...
ls -ld /etc
test -e /etc && echo "practice directory exists"
printf 'lab 66c task 2 warm-up at %s
...
Destroy-restore drill for Setup Port Forwarding DNAT. This task connects `firewall-cmd rich rules` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-66-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-66-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 66c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 66c Task 2
What I practiced: Destroy-restore drill for Setup Port Forwarding DNAT
