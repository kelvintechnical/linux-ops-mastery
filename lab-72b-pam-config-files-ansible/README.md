# Lab 72b - Explore PAM Config Files (Ansible)
...
## Task 1 - Ansible equivalent for Explore PAM Config Files
...
printf 'lab 72b task 1 warm-up at %s
...
Ansible equivalent for Explore PAM Config Files. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-72b.yml" <<'EOF'
---
- name: Lab 72b Task 1 - Explore PAM Config Files
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Stage PAM example safely
      ansible.builtin.copy:
        ...
        mode: '0644'
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-72b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
printf 'Persistence check 72b task 1: PASS
...
Lab 72b Task 1
What I practiced: Ansible equivalent for Explore PAM Config Files
...
## Task 2 - Ansible trap rehearsal for Explore PAM Config Files
...
ls -ld /opt
test -e /opt && echo "practice directory exists"
printf 'lab 72b task 2 warm-up at %s
...
Ansible trap rehearsal for Explore PAM Config Files. This task connects `ansible.builtin.copy / ansible.builtin.lineinfile boundary pattern` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-72b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
printf 'Persistence check 72b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 72b Task 2
What I practiced: Ansible trap rehearsal for Explore PAM Config Files
