# Lab 69a - Restrict Access via hosts.deny (RHCSA)
...
- PRIMARY COMMANDS: `/etc/hosts.deny`
- TRAPS: T69-A: hosts.deny can lock out wrapper-aware services if hosts.allow is not prepared first; T69-B: TCP Wrappers do not protect services that are not linked to libwrap; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=69
export LAB_SLUG=hosts_deny_restrictions
...
## Task 1 - Canonical RHCSA form for Restrict Access via hosts.deny
...
printf 'lab 69a task 1 warm-up at %s
...
Canonical RHCSA form for Restrict Access via hosts.deny. This task connects `/etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 69 evidence for Restrict Access via hosts.deny\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: /etc/hosts.deny\n' | tee -a ${SANDBOX}/task-1-topic.txt
...
Task 1 is the safe, exam-style version of Restrict Access via hosts.deny. It creates evidence first and avoids changing production state without a restore path.
...
| Concept | Answer |
|---|---|
| Command family | `/etc/hosts.deny` |
| Practice directory | `/root` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T69-A: hosts.deny can lock out wrapper-aware services if hosts.allow is not prepared first |
...
printf 'Persistence check 69a task 1: PASS
...
Lab 69a Task 1
What I practiced: Canonical RHCSA form for Restrict Access via hosts.deny
Trap rehearsed: T69-A: hosts.deny can lock out wrapper-aware services if hosts.allow is not prepared first
...
## Task 2 - Trap rehearsal and contrast for Restrict Access via hosts.deny
...
ls -ld /root
test -e /root && echo "practice directory exists"
printf 'lab 69a task 2 warm-up at %s
...
Trap rehearsal and contrast for Restrict Access via hosts.deny. This task connects `/etc/hosts.deny` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T69-B: TCP Wrappers do not protect services that are not linked to libwrap\n' | tee ${SANDBOX}/task-2-trap.txt
...
| Practice directory | `/root` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T69-B: TCP Wrappers do not protect services that are not linked to libwrap |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 69a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 69a Task 2
What I practiced: Trap rehearsal and contrast for Restrict Access via hosts.deny
Trap rehearsed: T69-B: TCP Wrappers do not protect services that are not linked to libwrap
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 69 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
