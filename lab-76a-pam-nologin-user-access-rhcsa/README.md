# Lab 76a - Use PAM to Limit User Access (RHCSA)
...
- PRACTICE DIRECTORY: `/sys`
- PRIMARY COMMANDS: `/etc/nologin`
- TRAPS: T76-A: leaving /etc/nologin behind blocks regular users after the lab; T76-B: root can still log in while regular users are blocked; T41 destroy-restore; T44 cleanup safety

This lab's practice directory is: `/sys`.
...
```bash
export LAB_NUM=76
export LAB_SLUG=pam_nologin_user_access
...
Practice directory: /sys
Kernel hardware and module state appears here. Writing here can change live kernel behavior.
EOF
id "${USER}"
ls -ld "${SANDBOX}" "${USER_HOME}" /sys
echo "Sandbox built by $(whoami) at $(date -Is)"
echo "exit was: $?"
```

## Task 1 - Canonical RHCSA form for Use PAM to Limit User Access
...
pwd
ls -ld /sys
test -e /sys && echo "practice directory exists"
printf 'lab 76a task 1 warm-up at %s
...
Canonical RHCSA form for Use PAM to Limit User Access. This task connects `/etc/nologin` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Lab 76 evidence for Use PAM to Limit User Access\n' | tee ${SANDBOX}/task-1-topic.txt
printf 'Command family: /etc/nologin\n' | tee -a ${SANDBOX}/task-1-topic.txt
ls -ld /sys | tee -a "${SANDBOX}/task-1-practice.txt"
test -e /sys && echo "/sys checked again" | tee -a "${SANDBOX}/task-1-practice.txt"
...
Task 1 is the safe, exam-style version of Use PAM to Limit User Access. It creates evidence first and avoids changing production state without a restore path.
...
|---|---|---|
| `/etc/nologin` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `/etc/nologin` |
| Practice directory | `/sys` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T76-A: leaving /etc/nologin behind blocks regular users after the lab |
...
printf 'Persistence check 76a task 1: PASS
...
Lab 76a Task 1
What I practiced: Canonical RHCSA form for Use PAM to Limit User Access
Trap rehearsed: T76-A: leaving /etc/nologin behind blocks regular users after the lab
...
## Task 2 - Trap rehearsal and contrast for Use PAM to Limit User Access
...
ls -ld /sys
test -e /sys && echo "practice directory exists"
printf 'lab 76a task 2 warm-up at %s
...
Trap rehearsal and contrast for Use PAM to Limit User Access. This task connects `/etc/nologin` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
printf 'Trap rehearsal: T76-B: root can still log in while regular users are blocked\n' | tee ${SANDBOX}/task-2-trap.txt
printf 'Restore note: verify before cleanup\n' | tee -a ${SANDBOX}/task-2-trap.txt
ls -ld /sys | tee -a "${SANDBOX}/task-2-practice.txt"
test -e /sys && echo "/sys checked again" | tee -a "${SANDBOX}/task-2-practice.txt"
...
| Practice directory | `/sys` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T76-B: root can still log in while regular users are blocked |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 76a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 76a Task 2
What I practiced: Trap rehearsal and contrast for Use PAM to Limit User Access
Trap rehearsed: T76-B: root can still log in while regular users are blocked
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 76 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
