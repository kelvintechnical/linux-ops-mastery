# Lab 82c - Toggling SELinux Booleans (Verify)
...
## Task 1 - Audit evidence for Toggling SELinux Booleans
...
printf 'lab 82c task 1 warm-up at %s
...
Audit evidence for Toggling SELinux Booleans. This task connects `getsebool, setsebool -P` to a repeatable artifact in `${SANDBOX}`.
...
printf 'Persistence check 82c task 1: PASS
...
Lab 82c Task 1
What I practiced: Audit evidence for Toggling SELinux Booleans
...
## Task 2 - Destroy-restore drill for Toggling SELinux Booleans
...
ls -ld /sbin
test -e /sbin && echo "practice directory exists"
printf 'lab 82c task 2 warm-up at %s
...
Destroy-restore drill for Toggling SELinux Booleans. This task connects `getsebool, setsebool -P` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cp -a "${SANDBOX}/journal.txt" /tmp/lab-82-journal.backup
rm -rf "${SANDBOX}/restore-test"
mkdir -p "${SANDBOX}/restore-test"
cp -a /tmp/lab-82-journal.backup "${SANDBOX}/restore-test/journal.restored"
...
printf 'Persistence check 82c task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 82c Task 2
What I practiced: Destroy-restore drill for Toggling SELinux Booleans
