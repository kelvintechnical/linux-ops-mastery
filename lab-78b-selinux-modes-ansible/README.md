# Lab 78b - Managing SELinux Modes (Ansible)
...
## Task 1 - Ansible equivalent for Managing SELinux Modes
...
printf 'lab 78b task 1 warm-up at %s
...
Ansible equivalent for Managing SELinux Modes. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
cat > "${SANDBOX}/lab-78b.yml" <<'EOF'
---
- name: Lab 78b Task 1 - Managing SELinux Modes
  hosts: localhost
  connection: local
  gather_facts: false
  tasks:
    - name: Query SELinux state
      ...
      changed_when: false
EOF
ansible-playbook --check --diff "${SANDBOX}/lab-78b.yml" | tee "${SANDBOX}/ansible-check.txt"
...
| Trap risk | T78-A: setenforce is runtime-only and does not change /etc/selinux/config |
...
printf 'Persistence check 78b task 1: PASS
...
Lab 78b Task 1
What I practiced: Ansible equivalent for Managing SELinux Modes
Trap rehearsed: T78-A: setenforce is runtime-only and does not change /etc/selinux/config
...
## Task 2 - Ansible trap rehearsal for Managing SELinux Modes
...
ls -ld /media
test -e /media && echo "practice directory exists"
printf 'lab 78b task 2 warm-up at %s
...
Ansible trap rehearsal for Managing SELinux Modes. This task connects `ansible.posix.selinux / community.general.sefcontext / ansible.posix.seboolean as appropriate` to a repeatable artifact in `${SANDBOX}`.

### Main Command Block

```bash
ansible-playbook "${SANDBOX}/lab-78b.yml" | tee "${SANDBOX}/ansible-apply.txt"
...
| Practice directory | `/media` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T78-B: disabled mode cannot be enabled live without reboot |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 78b task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 78b Task 2
What I practiced: Ansible trap rehearsal for Managing SELinux Modes
Trap rehearsed: T78-B: disabled mode cannot be enabled live without reboot
What I will verify later: evidence file exists and the system state is restored.
EOF
```
