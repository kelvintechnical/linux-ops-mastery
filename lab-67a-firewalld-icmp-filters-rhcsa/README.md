# Lab 67a - Configure ICMP Filters (RHCSA)
...
- PRIMARY COMMANDS: `firewall-cmd --add-icmp-block`
- TRAPS: T67-A: blocking echo-request stops ping replies and can hide useful diagnostics; T67-B: runtime ICMP blocks vanish after reload unless permanent; T41 destroy-restore; T44 cleanup safety
...
```bash
export LAB_NUM=67
export LAB_SLUG=firewalld_icmp_filters
...
## Task 1 - Canonical RHCSA form for Configure ICMP Filters
...
printf 'lab 67a task 1 warm-up at %s
...
Canonical RHCSA form for Configure ICMP Filters. This task connects `firewall-cmd --add-icmp-block` to a repeatable artifact in `${SANDBOX}`.
...
sudo firewall-cmd --list-icmp-blocks | tee ${SANDBOX}/icmp.txt
...
Task 1 is the safe, exam-style version of Configure ICMP Filters. It creates evidence first and avoids changing production state without a restore path.
...
| `--add-icmp-block` | Practice token for this lab | Repeat it until recognition is automatic |

### Concept Card

| Concept | Answer |
|---|---|
| Command family | `firewall-cmd --add-icmp-block` |
| Practice directory | `/boot` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T67-A: blocking echo-request stops ping replies and can hide useful diagnostics |
...
printf 'Persistence check 67a task 1: PASS
...
Lab 67a Task 1
What I practiced: Canonical RHCSA form for Configure ICMP Filters
Trap rehearsed: T67-A: blocking echo-request stops ping replies and can hide useful diagnostics
...
## Task 2 - Trap rehearsal and contrast for Configure ICMP Filters
...
ls -ld /boot
test -e /boot && echo "practice directory exists"
printf 'lab 67a task 2 warm-up at %s
...
Trap rehearsal and contrast for Configure ICMP Filters. This task connects `firewall-cmd --add-icmp-block` to a repeatable artifact in `${SANDBOX}`.
...
sudo firewall-cmd --reload
sudo firewall-cmd --list-icmp-blocks | tee ${SANDBOX}/runtime-trap.txt
...
| Practice directory | `/boot` |
| Evidence directory | `${SANDBOX}` |
| Trap risk | T67-B: runtime ICMP blocks vanish after reload unless permanent |
| Recovery phrase | Stop, restore the saved state, then verify before continuing |
...
printf 'Persistence check 67a task 2: PASS
' | tee -a "${SANDBOX}/journal.txt"
```
...
cat >> "${SANDBOX}/journal.txt" <<'EOF'
Lab 67a Task 2
What I practiced: Trap rehearsal and contrast for Configure ICMP Filters
Trap rehearsed: T67-B: runtime ICMP blocks vanish after reload unless permanent
What I will verify later: evidence file exists and the system state is restored.
EOF
```
...
echo "Lab 67 cleanup complete at $(date -Is)"
echo "exit was: $?"
```
